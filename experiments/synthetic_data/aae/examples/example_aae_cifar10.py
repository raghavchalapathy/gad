...
def example_aae(path, adversarial_optimizer):
    ...
    model.adversarial_compile(adversarial_optimizer=adversarial_optimizer,
                              player_optimizers=[Adam(3e-5, decay=1e-5), Adam(1e-5, decay=1e-4)],
    ...
    image_and_inverted_anamolies_train = {'image1': 3, 'image2': 3, 'anomalies1': 3, 'imagecount': 2500,
                                          'anomaliesCount': 50, 'flipimage': "yes", 'slice_stich': "no"}
    [xtrain, Y] = prepare_cifar_data_with_anamolies(X, Y, image_and_inverted_anamolies_train)

    xtest = xtrain
    # ## Case 1 : (Normal: Cats ) , (Anommalies: Inverted Cats )
    # image_and_inverted_anamolies_train = {'image1': 3, 'image2': 3, 'anomalies1': 3, 'imagecount': 500,
    #                                       'anomaliesCount': 50, 'flipimage': "yes", 'slice_stich': "no"}
    # slice_and_stitch = {}
    #
    # xtrain, xtest = prepare_cifar_data_with_anamolies(image_and_inverted_anamolies_train,slice_and_stitch)

    # # Case 2 : (Normal: Cats , Dogs) , (Anommalies: sliced and stiched Cats and Dogs )
    # image_and_sliced_stiched_train = {'image1': 3, 'image2': 3, 'imagecount': 1000, 'flipimage': "no",
    #                                   'anomaliesCount': 50,'slice_stich': "yes"}
    # slice_and_stitch = {'image1': 3, 'image2': 5, 'imagecount1': 50, 'imagecount2': 50, 'anomaliesCount': 50}
    #
    # xtrain, xtest = prepare_cifar_data_with_anamolies(image_and_sliced_stiched_train,slice_and_stitch)
...
def main():
    # example_aae("output/aae-cifar10_anomalies_stiched", AdversarialOptimizerSimultaneous())
    example_aae("output/aae-cifar10_anomalies_rotated", AdversarialOptimizerSimultaneous())
