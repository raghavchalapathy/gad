...
image_and_inverted_anamolies_train = {'image1': 3,'image2': 3,'anomalies1':3,'imagecount': 2500,'anomaliesCount':50,'flipimage':"yes",'slice_stich':"no"}
...
# syntheticdataFilePath = "/Users/raghav/Documents/Uni/group_anomalies_detection/groupAnomalies/data/kdd_2018/Synthetic/Syn_Rotation.mat"
syntheticdataFilePath ="/Users/raghav/Documents/Uni/group_anomalies_detection/groupAnomalies/data/kdd_2018/Synthetic/Syn_Rotation_5000G.mat"
def prepare_synthetic_data_with_anamolies(dataPath):

    import scipy.io as sio
    mat_contents = sio.loadmat(dataPath)
    print mat_contents
    data = mat_contents['X_runs']
    # print "data shape",data.shape
    temp_label = 1 * np.ones(500)
    ana_labels = -1 *np.ones(50)
    datalabels = np.concatenate((temp_label, ana_labels), axis=0)

    return [data, datalabels]
[X,Y]=prepare_synthetic_data_with_anamolies(syntheticdataFilePath)

x_train = X.astype('float32') / 1.
x_train = x_train.reshape((x_train.shape[0],) + original_img_size)

basepath = "/Users/raghav/Documents/Uni/group_anomalies_detection/groupAnomalies/experiments/synthetic_data/vae/"
...
vae.save(basepath+'/models/cifar10_ld_%d_conv_%d_id_%d_e_%d_vae.h5' % (latent_dim, num_conv, intermediate_dim, epochs))
encoder.save(basepath+'/models/cifar10_ld_%d_conv_%d_id_%d_e_%d_encoder.h5' % (latent_dim, num_conv, intermediate_dim, epochs))
generator.save(basepath+'/models/cifar10_ld_%d_conv_%d_id_%d_e_%d_generator.h5' % (latent_dim, num_conv, intermediate_dim, epochs))
